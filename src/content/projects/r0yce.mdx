...
#### Diff Notation (Shiki + rehype-pretty-code)

The docs need to support a concise GitHub-style diff notation in code fences. You can write changes using `+` and `-` at the start of lines, or mark the fence with `diff` (for example: `ts diff`). The MDX pipeline runs a small transformer that:

- strips the leading `+`/`-` from rendered text (so copy/paste yields clean code)
- adds `.add` / `.remove` classes to line elements (so you can style additions/removals)
- scopes the block with a `.diff` class for targeted CSS

Example (write this in MDX):
...
- Transformer: `src/lib/shiki-diff-notation.ts`
- Pipeline: wired into `rehype-pretty-code` via `velite.config.ts`
- Styling: use `src/styles/mdx.css` to control `.add`, `.remove`, and `.diff` visuals; markers are rendered via CSS pseudo-elements and are non-selectable.

This notation preserves normal Shiki syntax highlighting while adding clear diff highlighting in the docs.
